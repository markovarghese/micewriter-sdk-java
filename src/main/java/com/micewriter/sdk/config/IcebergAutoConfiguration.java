...
public class IcebergAutoConfiguration {
    ...
    /**
     * The registrar is NOT {@code @ConditionalOnMissingBean} — always register it
     * so schema registration fires on every startup.
     */
    @Bean
    public SchemaRegistrar schemaRegistrar(UdsConnection connection, IcebergProperties props) {
        return new SchemaRegistrar(connection, props.getBasePackage());
    }
}
